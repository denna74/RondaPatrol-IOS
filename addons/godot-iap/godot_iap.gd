...
## GodotIap - iOS/macOS in-app purchase plugin for Godot
##
...
## [/codeblock]
##
## [b]Critical:[/b] Purchases must be finalized promptly.
...
func has_active_subscriptions_result(
	...
	return _active_subscription_failure(
		"not-prepared" if _is_apple() else "feature-not-supported",
...
## @return String - country code (e.g., "USA" on Apple)
##
## See: https://openiap.dev/docs/apis/get-storefront
func get_storefront() -> String:
	print("[GodotIap] get_storefront called")
	if not _native_plugin:
		var unavailable_code = "not-prepared" if _is_apple() else "feature-not-supported"
		purchase_error.emit({
...
## @return Types.VerifyPurchaseResultIOS or null on failure
##
